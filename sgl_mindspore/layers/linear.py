...
class QKVParallelLinear(ColParallelLinear):
    def __init__(
        ...
        self.tp_size = effective_tp_size
        self.num_heads = divide(self.total_num_heads, effective_tp_size)

        if effective_tp_size > self.total_num_kv_head:
            assert effective_tp_size % self.total_num_kv_head == 0
            self.num_kv_heads = 1
            self.num_kv_head_replicas = divide(self.tp_size, self.total_num_kv_head)
        else:
            self.num_kv_heads = divide(self.total_num_kv_head, effective_tp_size)
            self.num_kv_head_replicas = 1
        input_size = self.hidden_size
        output_size = (
            self.num_heads + 2 * self.num_kv_heads
        ) * effective_tp_size * self.head_dim
